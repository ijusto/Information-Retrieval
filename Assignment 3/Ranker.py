...
class Ranker:
    ...
    def proximity_boost(scoreDict, queryTerms):
        ...
        for term in range(len(queryTerms) -1):
            ...
            if (proximity_terms[0] in documentsInfo) & (proximity_terms[1] in documentsInfo):
                doc = []
                for docId in documentsInfo[proximity_terms[0]]:
                    score = 0
                    if docId in documentsInfo[proximity_terms[1]]:
                        score = check_proximity(proximity_terms[0], proximity_terms[1],
                                                           docId)
                    try:
        ...
        for k, v in proximity_score.iteritems():
            doclen = avgDocLen[k] # document's length
