...
class Evaluation:
    ...
    def __init__(self, rankType):
        self.rankType = rankType # REMOVE

    def getResults(self, file, queries, scores, start, end):
        ...
        # Metrics Calculation
        precision10, recall10, f_mesure10, avg_precision10, ndcgain10 = getCalculation(relevance_1, relevance_2, scores, queries, 10) # Top 10
        precision20, recall20, f_mesure20, avg_precision20, ndcgain20 = getCalculation(relevance_1, relevance_2, scores, queries, 20)  # Top 20
        precision50, recall50, f_mesure50, avg_precision50, ndcgain50 = getCalculation(relevance_1, relevance_2, scores, queries, 50)  # Top 50
        ...
        for idx, query in enumerate(queries):
            print('\n{:2.0f}    #'
                ...
                '    {:-10.2f}'.format(50, 
                    ...
                    (f_mesure10[idx]*100), (f_mesure20[idx]*100), (f_mesure50[idx]*100), 
                    (avg_precision10[idx]*100), (avg_precision20[idx]*100), (avg_precision50[idx]*100),  
        ...
        '    {:-6.2f}   {:-6.2f}   {:-6.2f}'
        '    {:-10.2f}'.format(50, 
            (statistics.mean(precision10)*100), (statistics.mean(precision20)*100), (statistics.mean(precision30)*100), 
            (statistics.mean(recall10)*100), (statistics.mean(recall20)*100), (statistics.mean(recall50)*100),
            (statistics.mean(f_mesure10)*100), (statistics.mean(f_mesure20)*100), (statistics.mean(f_mesure50)*100),
            (statistics.mean(avg_precision10)*100), (statistics.mean(avg_precision20)*100), (statistics.mean(avg_precision50)*100),
            (statistics.mean(ndcgain10)*100), (statistics.mean(ndcgain20)*100), (statistics.mean(ndcgain50)*100),
    ...
    def getCalculation(relevance_1, relevance_2, scores, queries, top):
        ...
        for num, query in enumerate(queries):
            ...
            if recall[num] != 0 or precision[num] != 0:
                f_mesure.append(((pow(beta, 2)+1) * recall[num]*precision[num]) / (recall[num] + (pow(beta, 2) * precision[num])))
            else:
                f_mesure.append(0)
            ...
            if idcg != 0:
                ...
                ndcgain.append(0)
        return  precision, recall, f_mesure, avg_precision, ndcgain
